...
const InfoDrodown = React.memo(
  ({ isObserver = false }: { isObserver?: boolean }) => {
    ...
    const memoizedRoomInfo = useMemo(() => {
      return {
        title: isObserver
          ? observerRoomInfoDetails.title
          : roomInfoDetails?.title,
        description: isObserver
          ? observerRoomInfoDetails.description
          : roomInfoDetails?.description,
        continentType: getKeyFromDbKey(
          isObserver
            ? observerRoomInfoDetails.continentType
            : roomInfoDetails?.continentType!
        ),
        categoryType: getKeyFromDbKey(
          isObserver
            ? observerRoomInfoDetails.categoryType
            : roomInfoDetails?.categoryType!
        ),
        speakCountTime: timeFormatter(
          isObserver
            ? observerRoomInfoDetails.speakCountType *
                observerRoomInfoDetails.timeType *
                2
            : roomInfoDetails?.speakCountType! * roomInfoDetails?.timeType! * 2
        ),
        memberNumberType: isObserver
          ? observerRoomInfoDetails.memberNumberType
          : roomInfoDetails?.memberNumberType,
        resultEnabled: getKeyFromDbKey(
          isObserver
            ? observerRoomInfoDetails.resultEnabled
            : roomInfoDetails?.resultEnabled!
        ),
    ...
    useEffect(() => {
      console.log(
        "RoomInfoDetial 제대로 컴포넌트에 들어오는지 확인하기",
        roomInfoDetails,
        ...
        {infoOpen && (
          <div className="absolute md:left-10 md:w-[600px] w-[325px] left-6 h-auto py-[20px] px-[36px] md:bg-neutral-50/50 bg-white rounded-lg border border-white01  flex flex-col justify-between gap-[20px] md:shadow-[0px_1px_5px_0px_rgba(251,251,251,1.00)] z-50">
            ...
            <div className="flex justify-start flex-wrap gap-[10px] text-black01 font-bold">
              ...
              <div className="h-7 px-2.5 py-1 md:bg-neutral-50/70 rounded-3xl border border-gray03 justify-start items-center gap-2 inline-flex">
                <p>
                  {memoizedRoomInfo.memberNumberType} :{" "}
                  {memoizedRoomInfo.memberNumberType}
                </p>
              </div>
              <div className="h-7 px-2.5 py-1 md:bg-neutral-50/70 rounded-3xl border border-gray03 justify-start items-center gap-2 inline-flex">
